...
def get_threshold(M, epsilon, b, weight, n_topics):
    ...
    while True:
        ...
        if tmp_threshold > M * b * n_topics/weight:
            break 
        if tmp_threshold >= M:
            threshold[j] = tmp_threshold
    return threshold    
def streaming(model,b,epsilon,alpha,n_topics=3):
    start_time=time.time()
    M=0.0
...
for b in B:
    f_str,count_f_str,time_str=streaming(modelICM,b,epsilon,alpha,n_topics=n_topics)
